...
class MenuItem:
    ...
    description: str
    can_order: bool
    day : 'MenuDay' = field(repr=False, default=None)
...
class Primirest:
    ...
    def __auth(self, username: str, password: str):
        ...
        if response.ok:
            ...
            if len(soup.select("body > div.horni-panel > div.horni > div.vitejte-v > div.welcome")) > 0: # prvek pro kontrolu prihlaseni
                ...
                user_id, user_name, user_info = self.__get_user()
                ...
                self.boarder = Boarder(id=user_id, name=user_name, info=user_info, balance=user_balance, sum_of_orders=user_sum_of_orders, balance_real=user_balance_real)
    ...
    def __get_user(self):
        ...
        if req.ok:
            ...
            return user_data["ID"], user_data["Name"], user_data["AdditionalInfo"]
        else:
    ...
    def __get_menu_days_and_items(self, menu: Menu):
        ...
        req = self.session.get(url)
        if req.ok:
            for day in json.loads(req.text)["Menu"]["Days"]:
                if len(day["Items"]) > 0:
                    id_day = int(day["Items"][0]["IDMenuDay"])
                    day_date = datetime.fromtimestamp(int(re.search(r'\d+', day["Items"][0]["MenuDayDate"]).group()) / 1000).date()
                    menu_day = MenuDay(id=id_day, date=day_date, menu=menu)

                    for item in day["Items"]:
                        item_id = int(item["Flags"][0]["IDEntity"])
                        item_description = item["Description"]
                        item_can_order = bool(item["CanOrder"])
                        menu_item = MenuItem(id=item_id, description=item_description, can_order=item_can_order, day=menu_day)
                        menu_day.items.append(menu_item)
                    
                    menu.days.append(menu_day)
                    
    def get_boarder_consumptions(self):
        url = f"https://mujprimirest.cz/ajax/cs/consumptions/{self.boarder.id}/index?id={self.boarder.id}&from=01.01.1900&to=01.01.3000&_={self.__ts()}"
        req = self.session.get(url)
        consumptions = []
        if req.ok:
    ...
    def order(self, item: MenuItem):
        url = f"https://mujprimirest.cz/ajax/CS/boarding/0/order?menuID={item.day.menu.id}&dayID={item.day.id}&itemID={item.id}&purchasePlaceID={item.day.menu.purchase_place.id}&_={self.__ts()}"
        req = self.session.get(url)
        if req.ok:
